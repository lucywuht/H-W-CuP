...
Treat both as first-class Living Design components. Use the component that best matches intent and domain context. For the component catalogue (name, category, import path, intent), skim `components-index.instructions.md`. For prop API and usage notes on any specific component, open the `<Name>.md` next to its `.tsx` in `src/components/<dir>/` (both files are generated and read-only) or run `npx ld-kit show <Name>`. For theme and brand setup, rely on `theming.instructions.md`.
...
**Step 2: Skim the Component Index (REQUIRED)**
Read `components-index.instructions.md` in the `.github/instructions/` directory. It lists every available Living Design component with category, import path, and one-line intent. Use it to shortlist candidates by intent.

For each component you plan to use, open the `<Name>.md` next to its `.tsx` in `src/components/<dir>/` (the `<dir>` matches the import path — e.g. `Button.md` and `ButtonGroup.md` both live in `src/components/Button/`) **or** run `npx ld-kit show <Name>` from the project root. Do this on demand — only for components you actually intend to use.
...
**Full-bleed exceptions**: Headers (`Header`) and footers (`DesktopFooter`, `MwebFooter`) are the ONLY elements that should sit outside `Container`. Everything else — carousels, banners, card grids, product sections — goes inside `Container`.
...
ALWAYS prefer existing `patterns/` components over building custom equivalents. These patterns already handle responsive layout, spacing, and accessibility internally.
...
**Hard rule**: Before building any custom section (footer, header, product row, banner), check `components-index.instructions.md` (and open the candidate's `<Name>.md` next to its `.tsx` — at `src/components/<Name>/` or `src/patterns/<Name>/`) for a pattern that already does it. Build custom only as a last resort.
