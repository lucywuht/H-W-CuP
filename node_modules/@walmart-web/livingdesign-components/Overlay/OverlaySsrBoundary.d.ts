import * as React from "react";
export declare type OverlaySsrBoundaryProps = React.ComponentPropsWithoutRef<"div">;
/**
 * @private
 *
 * Boundary to prevent rendering of nested components during SSR. (This is necessary for
 * components that access DOM elements, e.g., component adds a portal element to the DOM.)
 *
 */
export declare const OverlaySsrBoundary: React.FunctionComponent<OverlaySsrBoundaryProps>;
