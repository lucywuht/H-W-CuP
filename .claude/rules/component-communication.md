...
`FlashDealsCarousel` is a fixed-data organism. Its built-in `+ Add` actions write directly to `window.ldKit.items.addToCart(...)`, so header cart totals still update when the component is dropped onto a page.
