...
Read `components-index.md` in the `.claude/rules/` directory. It lists every available Living Design component with category, import path, and one-line intent. Use it to shortlist candidates by intent.
...
**Hard rule**: Before building any custom section (footer, header, product row, banner), check `components-index.md` (and open the candidate's `<Name>.md` next to its `.tsx` — at `src/components/<Name>/` or `src/patterns/<Name>/`) for a pattern that already does it. Build custom only as a last resort.
