...
**Import:** `import { TextFieldSplit } from "./components/TextFieldSplit"`
**Category:** components
...
- `selectA11yLabel`: string (required) — The visually hidden accessible label for the leading select. The visible
- `selectChildren`: ReactNode (required) — The option elements rendered inside the leading select.
- `selectOnChange`: (event: ChangeEvent<HTMLSelectElement>) => void (required) — The callback fired when the leading select changes.
- `selectProps`: ComponentPropsWithoutRef<"select"> — Props spread to the leading select element.
- `selectValue`: string — The value for the leading select.
- `size`: TextFieldSize — The size for the text field.
...
- `type`: TextFieldInputType — The type for the text field.
