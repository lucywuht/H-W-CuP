...
- `href`: string (required) — The href for the tab navigation item.
- `isCurrent`: boolean — If the tab navigation item represents the current page.
- `leadingIcon`: ReactNode — The leading icon for tab navigation item.
- `onClick`: (event: MouseEvent<HTMLAnchorElement>) => void — The callback fired when the tab navigation item is clicked.
- `target`: string — The target for the tab navigation item.
